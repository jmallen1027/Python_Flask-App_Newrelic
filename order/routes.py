...
#logging.basicConfig(filename='orders.log', level=INFO, format=f'%(asctime)s %(levelname)s %(name)s %(threadName)s : %(message)s')

order_blueprint = Blueprint('order_api_routes', __name__,
                          url_prefix='/api/order')


#USER_API_URL = 'http://user-service-c:5001/api/user'
#USER_API_URL = 'http://127.0.0.1:5001/api/user'
USER_API_URL = 'http://user-app:5001/api/user'
